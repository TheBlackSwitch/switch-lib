...
execute unless data storage theblackswitch:overlay data[3] run return:
    execute unless items entity @a[predicate=theblackswitch:player_id/match_search,limit=1] armor.feet *[minecraft:custom_data~{"tbs.overlay":{"applied":1b}}] run return fail

    execute if data entity @s Items[{Slot:3b,components:{"minecraft:custom_data":{"tbs.clear_inventory":true}}}] run return:
        item replace entity @a[predicate=theblackswitch:player_id/match_search,limit=1] armor.feet with air
    ...
    execute if data entity @s Items[{Slot:3b,components:{"minecraft:custom_data":{"tbs.overlay":{equippable: "none"}}}}]:
        data remove entity @s Items[{Slot:3b}].components."minecraft:equippable"

    # Swap the equippable component if there was one present before
    execute unless data entity @s Items[{Slot:3b,components:{"minecraft:custom_data":{"tbs.overlay":{"equippable":"none"}}}}]:
        data modify entity @s Items[{Slot:3b}].components."minecraft:equippable" set from entity @s Items[{Slot:3b}].components."minecraft:custom_data"."tbs.overlay".equippable

    data remove entity @s Items[{Slot:3b}].components."minecraft:custom_data"."tbs.overlay"

    # move the modified item back to the player
    item replace entity @a[predicate=theblackswitch:player_id/match_search,limit=1] armor.feet from entity @s container.3
...
if score #should_modify temp matches 1:
    ...
    function theblackswitch:overlay/show/reconstruct_equippable/feet

    # copy the normal equippable component to the item if it doesn't aready exist
    execute unless data entity @s Items[{Slot:3b}].components."minecraft:custom_data"."tbs.overlay".equippable:
    ...
    data modify entity @s Items[{Slot:3b}].components."minecraft:custom_data"."tbs.overlay".applied set value true # mark this item as modified by the overlay
